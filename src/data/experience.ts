...
export const experience: ExperienceItem[] = [
  ...
  },
  {
    ...
    description: `
    Built RatePilot to track Swedish mortgage rates and the Riksbank policy rate over time, and to make comparing
    lenders easier than digging through PDFs and bank websites.

    It pulls from three different sources: the Riksbank's policy rate API, each lender's advertised list rates
    (scraped directly from their sites), and the average rate banks are required to publish monthly under
    Finansinspektionen rules. Everything runs as scheduled jobs that write to plain data files committed to git,
    no database, no servers running around the clock. The site itself is a static build that reads that data
    directly.

    The interesting part wasn't really the scraping, it was designing something that stays reliable when a bank
    redesigns their page or drops a PDF format overnight. Each source fails independently, gets logged, and never
    takes down the rest of the pipeline.

    Live at ratepilot.se with rate history, lender comparisons, and a few calculator tools for people trying to
    figure out if they should switch banks or renegotiate.
`,
    tags: [
      {
        category: "Sources",
        tags: [
          "Riksbank Policy Rate API",
          "Web Scraping",
      ...
      },
      {
        ...
        tags: ["Scheduled Jobs", "Static Site", "Git as Data Store"],
      },
    ],
  },
  {
    company: "Andy - your AI stylist",
    link: "https://andy.style",
    role: "Founder / Independent Product Developer",
    start: new Date(2026, 3, 1),
    description: `
    Andy is an AI stylist for the wardrobe you already own. You photograph your clothes, the app catalogs each
    piece automatically (category, colour, fabric), and it suggests outfits based on your taste, the weather, and
    where you're headed that day. The point is to get more wear out of what's already in your closet instead of
    buying more.

    I built and shipped it end to end: the mobile app, the API, the marketing site, and the infrastructure behind
    them.

    On the technical side, it's a single Bun monorepo. The app is React Native on Expo, the API is Elysia with
    PostgreSQL and Drizzle, and the marketing site is Astro. Types flow directly from the backend into the mobile
    app through Eden Treaty, so there's no hand-written API schema to keep in sync.

    The outfit suggestions come from a tool-calling agent built on Google Gemini. It reads the user's actual
    wardrobe, checks the weather for their location, and assembles a specific outfit from real items rather than
    handing back generic advice. Image classification and ghost-mannequin image generation run as background jobs
    off a queue, which keeps photo uploads quick.

    The rest of the stack: better-auth for email and Apple/Google sign-in, S3-compatible object storage for
    wardrobe images, RevenueCat for subscriptions alongside a credit system for AI usage, and Sentry across both
    the backend and the app.

    Available on the App Store and Google Play, and more info on andy.style.
`,
    tags: [
      {
      ...
      {
        category: "AI",
        tags: ["Google Gemini", "Tool-Calling Agents", "Image Generation"],
      },
      {
        category: "Other Services",
        tags: ["better-auth", "S3", "RevenueCat", "Sentry"],
      },
    ],
  },
  {
